...
// -------------------------
export const glRebuild = async ({ startDate, endDate, branchIdOrZoneId, dryRun = false, settlementMode, historicalStockFundingMode, historicalCogsMode, forceExpensePaymentAccount, accountingPolicy } = {}) => {
  try {
    ...
    const params = {
      ...
      ...(historicalStockFundingMode ? { historicalStockFundingMode } : {}),
      ...(historicalCogsMode ? { historicalCogsMode } : {}),
      ...(forceExpensePaymentAccount ? { forceExpensePaymentAccount } : {}),
...
export const glListApprovedUnposted = async ({ startDate, endDate, branchIdOrZoneId, status = 'ALL', page = 1, limit = 25, includeOrders = false } = {}) => {
  try {
    requireRange(startDate, endDate);
    const params = {
      startDate,
      ...
      page,
      limit,
      includeOrders: includeOrders ? 'true' : 'false',
      ...withBranchParams(branchIdOrZoneId),
    };
...
// -------------------------
export const glReadiness = async ({ startDate, endDate, branchIdOrZoneId, includeOrders = false } = {}) => {
  try {
    const params = {
      ...(hasVal(startDate) ? { startDate } : {}),
      ...(hasVal(endDate) ? { endDate } : {}),
...
// -------------------------
export const startGlRebuildJob = async ({ startDate, endDate, branchIdOrZoneId, dryRun = true, settlementMode = 'SAME_DAY', historicalStockFundingMode = 'BANK', historicalCogsMode = 'STOCK_PURCHASE_FULL_COST', forceExpensePaymentAccount = 'BANK', accountingPolicy = 'OPTION_C_BANK_CENTRIC_WORKING_CAPITAL', includeOrders = false } = {}) => {
  try {
    requireRange(startDate, endDate);
    const payload = {
      startDate,
      endDate,
      branchIdOrZoneId: branchIdOrZoneId || undefined,
    ...
    const res = await httpClient.post('/gl/rebuild/jobs', payload);
    return res.data;
  } catch (err) {
    normalizeApiError(err, 'Failed to start GL rebuild job');
  }
};

export const getGlRebuildJob = async (jobId) => {
  try {
    const res = await httpClient.get(`/gl/rebuild/jobs/${jobId}`);
    return res.data;
  } catch (err) {
    normalizeApiError(err, 'Failed to fetch GL rebuild job');
  }
};

export const listGlRebuildJobs = async () => {
  try {
    const res = await httpClient.get('/gl/rebuild/jobs');
    return res.data;
  } catch (err) {
    normalizeApiError(err, 'Failed to fetch GL rebuild jobs');
  }
};

// -------------------------
// Background GL reset jobs
// -------------------------
export const startGlResetJob = async ({ startDate, endDate, branchIdOrZoneId, dryRun = true, includeOrders = false } = {}) => {
  try {
    requireRange(startDate, endDate);
    const payload = {
      startDate,
    ...
    const res = await httpClient.post('/gl/reset/jobs', payload);
    return res.data;
  } catch (err) {
    normalizeApiError(err, 'Failed to start GL reset job');
  }
};

export const getGlResetJob = async (jobId) => {
  try {
    const res = await httpClient.get(`/gl/reset/jobs/${jobId}`);
    return res.data;
  } catch (err) {
    normalizeApiError(err, 'Failed to fetch GL reset job');
  }
};

export const listGlResetJobs = async () => {
  try {
    const res = await httpClient.get('/gl/reset/jobs');
    return res.data;
  } catch (err) {
    normalizeApiError(err, 'Failed to fetch GL reset jobs');
  }
};


export const verifyGlReset = async ({ startDate, endDate, branchIdOrZoneId, includeOrders = false } = {}) => {
  try {
    requireRange(startDate, endDate);
    const params = {
      startDate,
    ...
    const res = await httpClient.get('/gl/reset/verify', { params });
    return res.data;
  } catch (err) {
    normalizeApiError(err, 'Failed to verify GL reset state');
  }
};
