...
import { normalizeBranchOptions, filterBranchesForUser, chooseDefaultBranch, getBranchValue, getBranchLabel, branchAccessMessage } from '../../utils/branchAccess';
    ...
    useEffect(() => {
        const init = async () => {
            ...
            try {
                ...
                const normalizedPlants = normalizeBranchOptions(plantList);
                const scopedPlants = filterBranchesForUser(normalizedPlants, user);
                setPlants(scopedPlants);

                const defaultBranch = chooseDefaultBranch(normalizedPlants, user);
                setSelectedBranch((current) => {
                    const stillAllowed = scopedPlants.some((branch) => getBranchValue(branch) === current);
                    return stillAllowed ? current : (defaultBranch ? getBranchValue(defaultBranch) : '');
                });

                const accessWarning = branchAccessMessage(normalizedPlants, user);
                if (accessWarning) {
                    setNotification({ show: true, message: accessWarning, type: 'error' });
                }
            } catch (e) {
                console.error("Init Error", e);
                setNotification({ show: true, message: e?.message || 'Failed to load assigned branch list.', type: 'error' });
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-4">
                    <select 
                        ...
                        className="glass-input p-2 text-sm bg-black/20 w-56"
                        disabled={!plants.length}
                    >
                        {!plants.length && <option value="">No assigned branch</option>}
                        {plants.map(p => <option key={getBranchValue(p)} value={getBranchValue(p)}>{getBranchLabel(p)}</option>)}
                    </select>
            ...
            {!plants.length && (
                <div className="glass-card border-l-4 border-red-500 text-sm text-red-200">
                    This cashier has no valid assigned branch from the Operations branch list. Update Staff Access with an Operations branch/plant, then log out and back in.
                </div>
            )}
