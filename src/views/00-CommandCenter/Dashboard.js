...
export default function Dashboard({ setActiveView }) {
    ...
    const fetchData = async () => {
        ...
        try {
            const results = await Promise.allSettled([
                getGasPlantDashboard({ period, startDate: period === 'custom' ? customStart : undefined, endDate: period === 'custom' ? customEnd : undefined }),
                getOnlineDrivers(),
                getDailyClosePerformance({ period: period === 'custom' || period === 'allTime' ? 'yearly' : period, startDate: period === 'custom' ? customStart : undefined, endDate: period === 'custom' ? customEnd : undefined }),
            ]);
    ...
    const backendRange = dashboard?.period ? `${fmtRangeDate(dashboard.period.start)} → ${fmtRangeDate(dashboard.period.end)}` : '—';
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <PeriodButton active={period === 'custom'} label="Custom Range" onClick={() => setPeriod('custom')} />
            ...
            <div className="glass-card p-4 border border-white/10">
                <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                    <div>
                        <p className="text-xs uppercase tracking-wider text-gray-500 font-bold">Date basis</p>
                        <p className="text-sm text-gray-300 mt-1">Dashboard uses transaction/business dates, not import/upload dates. Backend range: <span className="text-white font-bold">{backendRange}</span></p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 min-w-full lg:min-w-[520px]">
                        <label className="text-xs text-gray-400">
                            Start Date
                            <input type="date" className="glass-input w-full p-2 mt-1" value={customStart} disabled={period !== 'custom'} onChange={(e) => setCustomStart(e.target.value)} />
                        </label>
                        <label className="text-xs text-gray-400">
                            End Date
                            <input type="date" className="glass-input w-full p-2 mt-1" value={customEnd} disabled={period !== 'custom'} onChange={(e) => setCustomEnd(e.target.value)} />
                        </label>
                        <button
                            className="px-3 py-2 rounded-xl bg-blue-600/80 hover:bg-blue-600 text-white text-xs font-bold border border-blue-400/30 disabled:opacity-50"
                            disabled={period !== 'custom'}
                            onClick={fetchData}
                        >
                            Recalculate Custom Range
                        </button>
                    </div>
                </div>
                {period === 'yearly' ? (
                    <div className="mt-3 text-xs text-amber-200 bg-amber-500/10 border border-amber-500/20 rounded-xl p-3">
                        <span className="font-bold">This Year</span> means Jan 1 of the current calendar year to today. To validate the full migrated period from June 2025 to date, use <span className="font-bold">Custom Range</span> or <span className="font-bold">All Migrated Data</span>.
                    </div>
                ) : null}
            </div>
