  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 bg-slate-900/60 border border-white/10 rounded-2xl p-4">
        <label className="text-sm text-slate-300">Start Date<input className="mt-1 w-full bg-slate-950 border border-white/10 rounded-xl p-2" type="date" value={filters.startDate} onChange={(e)=>setFilters({...filters,startDate:e.target.value})}/></label>
        <label className="text-sm text-slate-300">End Date<input className="mt-1 w-full bg-slate-950 border border-white/10 rounded-xl p-2" type="date" value={filters.endDate} onChange={(e)=>setFilters({...filters,endDate:e.target.value})}/></label>
        ...
        <Panel title="Recent GL Journals" icon={RotateCcw}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {journals.map((j) => <tr key={j._id} className="border-t border-white/5"><td className="p-2">{fmt(j.date || j.createdAt)}</td><td className="p-2 text-slate-300">{j.sourceType || j.narration || 'Journal'}</td><td className="p-2 text-right">{money(j.totalDebit || j.debit || j.debitAmount)}</td><td className="p-2 text-right">{money(j.totalCredit || j.credit || j.creditAmount)}</td><td className="p-2"><Badge tone={String(j.status).toUpperCase()==='POSTED'?'good':'neutral'}>{j.status || 'POSTED'}</Badge></td><td className="p-2 text-right"><button disabled={busy || String(j.status).toUpperCase()==='REVERSED'} onClick={()=>requestReverse(j)} className="px-3 py-1.5 rounded-lg bg-amber-600 disabled:bg-slate-700 text-white text-xs">Request reversal</button></td></tr>)}
