        ...
        <>
          ...
          <Card className="bg-white/5 border border-white/10 rounded-xl p-4">
            <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
              <div>
                <div className="text-lg font-bold text-white flex items-center gap-1">Approved but Unposted Entries <HelpTooltip text="Source records waiting for GL posting. Use this to monitor progress before and after Rebuild GL / Retry Failed." /></div>
                <div className="text-xs text-gray-400 mt-1">These are operational records for the selected period that are not fully posted to GL.</div>
              </div>
            ...
            <div className="mt-3 flex flex-col md:flex-row md:items-center md:justify-between gap-2 text-xs text-gray-400">
              <div>Total: {(unposted?.total || 0).toLocaleString()} • Failed: {(unposted?.counts?.failed || 0).toLocaleString()}</div>
              <div className="flex items-center gap-2">
                <span>Rows</span>
